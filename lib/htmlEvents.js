var htmlEvents = {
	...
	loggedIn : function(){
		...
		document.getElementById('info').innerHTML='<li><button class=\'btn btn-success\' href="#outputMobile" onClick=\'log("Your key is : '+config.pubkey+' ")\'> Show my key </button></li>';
	...
	showGroups : function(){
		...
		for (i in config.groupList){
			if (i==0) {
				...
				modalMenuString += '<li class="active"><a href="#'+config.groupList[i].name+'" data-toggle="tab">'+config.groupList[i].name+'<button class="btn btn-link" onClick="listManager.removeGroup('+i+');"><i class="icon-trash"></i></button></a></li>';
			} else{
				modalContentString += '<div class="tab-pane" id="'+config.groupList[i].name+'">';
				modalMenuString += '<li><a href="#'+config.groupList[i].name+'" data-toggle="tab">'+config.groupList[i].name+'<button class="btn btn-link" onClick="listManager.removeGroup('+i+');"><i class="icon-trash"></i></button></a></li>';
			}
