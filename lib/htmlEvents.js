var htmlEvents = {
	...
	showGroups : function(){
		...
		for (i in config.groupList){
			...
			modalMenuString += '<li><a href="#'+config.groupList[i].name+'" data-toggle="tab">'+config.groupList[i].name+'</a><button class="btn btn-link" onClick="listManager.removeGroup("'+config.groupList[i].name+'");"><i class="icon-trash"></i></button></li>'
