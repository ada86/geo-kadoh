var htmlEvents = {
	...
	loggedIn : function(){
		...
		document.getElementById('loggedInMobile').innerHTML='Logged in as '+config.name+'<br><br><button href"#outputMobile" class=\'btn btn-success\' onClick=\'log("Your key is : '+config.pubkey+' ")\'> Show my key </button>';
	...
	showGroups : function(){
		...
		for (i in config.groupList){
			...
			for (j in config.groupList[i].friendList) {
				if (typeof config.groupList[i].friendList[j] == "object")
