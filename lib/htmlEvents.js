var htmlEvents = {
	...
	loggedIn : function(){
		...
		document.getElementById('loggedInMobile').innerHTML='Logged in as <a href="#" onClick="log2()">'+config.name+'</a><br><br><button href"#outputMobile" class=\'btn btn-success\' onClick=\'prompt("Your key is :", config.pubkey)\'> Show my key </button>';
		document.getElementById('info').innerHTML='<li><button class=\'btn btn-success\' href="#outputMobile" onClick=\'prompt("Your key is :", config.pubkey)\'> Show my key </button></li>';
		...
		document.getElementById('hideMobile').className=""
	...
	showGroups : function(){
		...
		for (i in config.groupList){
			if (i==0) {
				...
				modalMenuString += '<li class="active"><a href="#'+config.groupList[i].name+'" data-toggle="tab">'+config.groupList[i].name+'<button class="btn-link" onClick="listManager.removeGroup('+i+');"><i class="icon-trash"></i></button></a></li>';
			} else{
				modalContentString += '<div class="tab-pane" id="'+config.groupList[i].name+'">';
				modalMenuString += '<li><a href="#'+config.groupList[i].name+'" data-toggle="tab">'+config.groupList[i].name+'<button class="btn-link" onClick="listManager.removeGroup('+i+');"><i class="icon-trash"></i></button></a></li>';
			}
