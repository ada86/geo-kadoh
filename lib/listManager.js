var listManager = {
	...
	removeGroup : function(g){
		...
		list = list.splice(0,list.indexOf(g))
			.concat(list.splice(list.indexOf(g)+1, list.length));
	...
	addFriendGroup : function(group, friend){
		...
		if (typeof group != "object"){alert("Group not found, did you spell it right ?");return;}
		...
		if (typeof group != "object"){alert("Friend not found, did you spell it right ?");return;}
