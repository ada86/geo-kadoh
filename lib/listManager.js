var listManager = {
	...
	removeFriend : function(friend){
		if (confirm("Are you sure you want to remove "+friend.name+"? If yes select OK, cancel otherwise.")){
	...
	removeGroup : function(g){
		if (confirm("Are you sure you want to remove "+config.groupList[g].name+"? If yes select OK, cancel otherwise.")){
	...
	addFriendGroup : function(group, friend){
		...
		group = document.getElementById('addFriendToGroup_group').value;
		friend = document.getElementById('addFriendToGroup_friend').value;

		if (group == 'Select Group' || friend == 'Select Friend') {return 1;}
	...
	removeFriendGroup : function(g,f){
		if (confirm("Are you sure you want to remove "+config.groupList[g].friendList[f].name+" from the group "+config.groupList[g].name+"? If yes select OK, cancel otherwise.")){
