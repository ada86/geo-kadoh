	...
	handleFriendLocation : function(value, friend){
		if (value != null){
			...
			if (position != null){
				...
				friend.lastPos = position;
				log(position.username + " is at : <a href='http://maps.google.com/maps?z=18&q="+position.coords.latitude+","+position.coords.longitude+"' target='_blank'>"+position.coords.latitude.toString().substring(0,6)+", "+position.coords.longitude.toString().substring(0,6)+"</a> ("+d.toUTCString()+")");
				showPosition(position.coords,false,friend.name + (typeof position.message != "undefined" ? " : "+position.message : " (no message attached)"));
				return;
			}
