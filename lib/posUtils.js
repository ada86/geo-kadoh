	...
	handleFriendLocation : function(value, friend){
		if (value != null){
			...
			if (position != null){
				...
				showPosition(position.coords,false,false);
				return;
			}
...
function showPosition(coords,add,name){
	...
	if (add == true) {
		...
		var map=new google.maps.Map(mapholder,myOptions);
		var mapMobile=new google.maps.Map(mapholderMobile,myOptions);
		setMarker(map,latlon,'http://maps.google.com/mapfiles/ms/icons/red-dot.png','You');
		setMarker(mapMobile,latlon,'http://maps.google.com/mapfiles/ms/icons/red-dot.png','You');
	}
}

function setMarker(map,pos,icon,name){
	var marker=new google.maps.Marker({position:pos,map:map,icon:icon});
	var infowindow = new google.maps.InfoWindow({content: '<p>'+name+'</p>'});
