var configManager = {
	...
	*/
		...
		try{//Check if user is in LocalStorage
			...
			try{
				node.get(Sha1.hash(username+password+username,false), function(value){
					try{
						...
						if (confirm("Wrong user/password. If you don't have an account, create one by selecting OK.")){
							createAccountGUI(username, password);
						}
						return;
					}
				});
				...
				if (confirm("There seem to be no connectivity. If you want to register a new account, create one by selecting OK.")){
					configManager.createAccountGUI(username, password);
					return;
				}
				return 1;
			}
	...
	createAccountGUI : function(username, password){
	...
	addFriendGUI : function(){
		...
		do{
			name	= prompt("Name of the friend");
		}while (name == "");
		if (name == null){return 1;}

		key 	= prompt("Key of the friend", key);
		configManager.addFriend(name,key);
		return;
	},
